...
from .utils import load_ontology,find_arg_span_original_text, compute_f1, get_entity_span, find_head, WhitespaceTokenizer,find_arg_span
...
def scorer(args):
    ...
    if args.coref:
        if args.dataset == 'wikievents' and args.coref_file:
    ...
    for key, ex in tqdm(items):
        ...
        for argname in predicted_args:
            for entity, score in predicted_args[
                argname]:  # this argument span is inclusive, FIXME: this might be problematic
                if len(entity) == 0:
                    continue
                ...
                for w in entity:
                    if w == 'and' and len(new_entity) > 0:
                        arg_span_original_text, arg_span = find_arg_span_original_text(new_entity, context_words,
                                                                                       trigger_start, trigger_end,
                                                                                       head_only=args.head_only,
                                                                                       doc=doc)
                        if arg_span:
                            predicted_set.add((arg_span[0], arg_span[1], evt_type, argname, score))
                            predicted_set_original_text.add(
                                (arg_span_original_text[0], arg_span_original_text[1], evt_type, argname, score))
                        new_entity = []
                    else:
                        new_entity.append(w)
    ...
    if len(pair_arg):
        print(
            f'paired args: {len(pair_arg)}, inconsistent args: {sum(pair_arg_inconsistent)}, raito: {sum(pair_arg_inconsistent) / len(pair_arg)}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset',type=str, default='wikievents', choices=['ACE', 'wikievents','AIDA'])
...
def scorer_rams(args):
    ...
    for key, ex in tqdm(items):
                ...
                for w in entity:
                    if w == 'and' and len(new_entity) > 0:
                        arg_span_original_text, arg_span = find_arg_span_original_text(new_entity, context_words,
                                                                                       ...
                                                                                       doc=doc)
                        if arg_span:
                            if argname in predicted_set:
                                predicted_set[argname].append((arg_span[0], arg_span[1]))
                            else:
                                predicted_set[argname] = list()
                                predicted_set[argname].append((arg_span[0], span[1]))
                            # predicted_set.add((arg_span[0], arg_span[1], evt_type, argname, score))
                            predicted_set_original_text.add(
                                (arg_span_original_text[0], arg_span_original_text[1], evt_type, argname, score))
                        new_entity = []
        ...
        # get gold spans
        # (span[0], span[1], evt_type, argname)
        gold_set = dict()
        event_arg = {}
        gold_canonical_set = set()  # set of canonical mention ids, singleton mentions will not be here
        for arg in ex['event']['arguments']:
            argname = arg['role']
            span = (arg['start'], arg['end'] - 1)
            span = clean_span(ex, span)
            # clean up span by removing `a` `the`
            # if args.head_only and span[0] != span[1]:
            #     span = find_head(span[0], span[1], doc=doc)
            if argname in gold_set:
                gold_set[argname].append((span[0], span[1]))
            else:
                gold_set[argname]=[]
                gold_set[argname].append((span[0], span[1]))
        ...
        for arg in ex['event']['arguments']:
            argname = arg['role']
            gt_list = gold_set[argname] if argname in gold_set else list()
            pred_list = list(set(predicted_set[argname])) if argname in predicted_set else list()
            gt_num += len(gt_list)
            pred_num += len(pred_list)
        ...
        for gt_span in all_gt_list:
            if gt_span in all_pred_list:
                correct_identify_num += 1
    res_c = eval_rpf(gt_num , pred_num, correct_num)
    res_i = eval_rpf(gt_num_identify, pred_num_identify, correct_identify_num)
    print('Classification: R {} P {} F {}'.format(
                  res_c['recall'],res_c['precision'], res_c['f1']))
    print('Identification: R {} P {} F {}'.format(
        res_i['recall'], res_i['precision'], res_i['f1']))
